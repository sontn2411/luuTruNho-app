...
import { Building2, Mail, Phone, MapPin, Tag, Check } from "lucide-react";
...
import { updateOrganization, type AdminOrgItem } from "@/app/admin/organizations/actions";

interface EditOrganizationModalProps {
  organization: AdminOrgItem | null;
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
}
...
export function EditOrganizationModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMsg(err?.message || "Đã xảy ra lỗi khi cập nhật tổ chức.");
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <DialogContent className="sm:max-w-[480px] p-6 space-y-4 rounded-2xl">
        <DialogHeader className="space-y-1 text-left">
          ...
          <DialogDescription className="text-xs text-muted-foreground">
            Cập nhật thông tin chi tiết cho tổ chức ({organization.name}).
          </DialogDescription>
        </DialogHeader>
